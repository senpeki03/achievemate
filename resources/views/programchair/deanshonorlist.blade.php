...
<style>
  ...
  /* visual selection for program row */
...
<div class="container py-4 page-root">
  ...
  <div id="students-container" class="d-none">
    ...
    <!-- Tabs + Download button aligned on one row -->
    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
      <ul class="nav nav-tabs mb-0" id="statusTabs" role="tablist">
        <li class="nav-item" role="presentation">
          <button class="nav-link active" id="tab-eval" data-status="For Evaluation" type="button" role="tab">
            For Evaluation
          </button>
        </li>
        <li class="nav-item" role="presentation">
          <button class="nav-link" id="tab-approval" data-status="For Approval" type="button" role="tab">
            For Approval
          </button>
        </li>
        <li class="nav-item" role="presentation">
          <button class="nav-link" id="tab-approved" data-status="Approved" type="button" role="tab">
            Approved
          </button>
        </li>
      </ul>

      <!-- Download button (enabled only on Approved tab + has data) -->
      <button id="download-report" class="btn btn-outline-primary btn-sm ms-auto" disabled
...
<script>
  ...
  window._allStudents = [];
  window._currentProgramId = null; // ← remember selected program
  ...
  document.getElementById('download-report').addEventListener('click', function () {
    const row = document.querySelector('#program-table tbody tr.selected, #program-table tbody tr.active');
    if (!row) { alert('Please select a program first.'); return; }

    const programId = row.dataset.id;

    const term = prompt('Enter Term (e.g., Second Semester):', '');
    if (!term) return;
    const ay = prompt('Enter Academic Year (e.g., 2024-2025):', '');
    if (!ay) return;

    const base = "{{ route('programchair.deanshonorlist.report', ':id') }}".replace(':id', programId);
    const url  = `${base}?term=${encodeURIComponent(term)}&ay=${encodeURIComponent(ay)}`;

    // Navigating the window (not opening a tab) cooperates best with browser download UI
  ...
  document.addEventListener('click', function(e){
    ...
    const programId = row.dataset.id;
    window._currentProgramId = programId;      // ← remember
    refreshDownloadBtn();
  ...
  function renderStudentsByStatus(status) {
    ...
    tb.innerHTML = '';
    rowCounter = 1;
    const list = (window._allStudents || []).filter(s => s.status === status);
    if (list.length === 0) {
  ...
  document.getElementById('confirmVerifyYesBtn').addEventListener('click', function () {
    ...
    fetch("{{ route('programchair.application.update-status') }}", {
      ...
      body: JSON.stringify({ id: _verifyContext.id, status: 'For Approval' })
    })
    .then(r => r.ok ? r.json() : r.text().then(t => { throw new Error(`HTTP ${r.status}: ${t}`); }))
    .then(() => {
      bootstrap.Modal.getInstance(document.getElementById('confirmVerifyModal')).hide();

      // Instant transfer to "For Approval"
  ...
  document.getElementById('verify-selected').addEventListener('click', function(){
    ...
    .then(r => r.ok ? r.json() : r.text().then(t => { throw new Error(`HTTP ${r.status}: ${t}`); }))
    .then(() => {
      // Update cache for all approved IDs
      ids.forEach(id => updateLocalStatus(id, 'For Approval'));

      // Clear selections and jump to For Approval
      document.getElementById('check-all').checked = false;
      refreshBulkButton();
      setActiveTab('For Approval');
  ...
  document.getElementById('back-button').addEventListener('click', function(){
    ...
    window._currentProgramId = null;  // clear selection
    refreshDownloadBtn();
  });
</script>
