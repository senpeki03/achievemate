{{-- resources/views/programchair/profile.blade.php --}}
@extends('programchair.programchairsidebar')
@section('title', 'Program Chair Profile | AchieveMate')
...
<script>
  ...
  document.getElementById('editBtn')?.addEventListener('click', async () => {
    ...
    try {
      const res = await fetch("{{ route('programchair.profile.save') }}", {
        method: 'POST',
      ...
      img.src = "{{ route('programchair.profile.photo') }}" + "?v=" + Date.now();
      selectedFile = null;
      alert('✅ Profile saved.');
    } catch (err) {
      alert('Failed to save profile.\n' + (err?.message || err));
